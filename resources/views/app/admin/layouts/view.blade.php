@extends('layouts.mediapartner.fileMaster')
  ...
  <div class="row">
   <div class="col-md-9 col-md-offset-1"><br>
      ...
      <div>
        ...
        <button class="btn btn-warning" id="back" type="button" >Back</button>
      </div>
   </div><!--end col-md 10 -->
...
// can.loadFromJSON(JSON.parse((<?= json_encode($layout->screen) ?>)));
  var obj = JSON.parse((<?= json_encode($layout->screen) ?>));
  var layout_resources = obj.objects;
  console.log(layout_resources);
  var name = ((<?= json_encode($layout->title) ?>));
  var color =(<?= json_encode($layout->background_color) ?>);
  
  document.getElementById('title').value = name;

  can.setBackgroundColor(color);

for (var i = layout_resources.length - 1; i >= 0; i--) {

  if((layout_resources[i].type)=="image"){

    if((layout_resources[i].src).match('.png') || (layout_resources[i].src).match('.jpg') ||(layout_resources[i].src).match('.jpeg')||(layout_resources[i].src).match('.gif')){
      image(layout_resources[i], 1, 1,can);
    }

  }else if((layout_resources[i].type)=="i-text"){   
   var text = new fabric.IText(layout_resources[i].text, {
      fill:layout_resources[i].fill,
      fontSize:(layout_resources[i].fontSize),
      fontFamily:layout_resources[i].fontFamily,
      left:(layout_resources[i].left),
      top:(layout_resources[i].top), 
      name:layout_resources[i].name,
      weblink:layout_resources[i].weblink,
      tag:"",
      content_id:0,                 
    });
  ...
  } else if((layout_resources[i].type)=="rect"){

     rectangle(layout_resources[i], 1, 1, can);         

  }else if((layout_resources[i].type)=="triangle"){

      triangle(layout_resources[i], 1, 1, can);

  }else if((layout_resources[i].type)=="circle"){

      circle(layout_resources[i], 1, 1, can);
...
$('#modal-youtube').on('click', 'button.ok', function(e){
  var url = ($(this).parent().parent('div').find('input:text').val());
  var content_id = parseInt(($(this).parent().parent('div').find('option:selected').data('content_id')));
...
$('#save-canvas').on('click', function(evt){
  ...
  var url = "{{url('mediapartner/'.$tenant->domain.'/layout/edit/'.$layout->id)}}";//{{url("view", $layout->id)}}
  ...
  $.ajax({
      ...
      success:function(data){
          window.location="{{url('mediapartner/'.$tenant->domain.'/layout/welcome')}}";
