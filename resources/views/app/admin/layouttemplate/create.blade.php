...
@section('content')
  ...
  <div class="row">
   <div class="col-md-9 col-md-offset-1"><br>
      <canvas id="draw" width="1000" height="700" style="border: 2px solid grey; "> </canvas> <br>
      <div>
  ...
  <div id="trial">
    ...
    <!-- <iframe width="420" height="315" src="https://www.youtube.com/embed/lou2vsfsHNs" frameborder="0" allowfullscreen></iframe> -->
  </div>
...
$('#modal-web').on('click', 'button.ok', function(e){
    var url = ($(this).parent().parent('div.modal-body').find('input:text').val());
   ...
   dragElement(document.getElementById(("resources")));
        var display = <?=$output = shell_exec('ls -lart');
        echo $output; ?>
...
can.on('object:selected', function(e) {
    ...
    $('back').on('click', function(evt){
        ...
        // {
        // can.sendBackwards(activeObject);
        // }
...
$('#save-canvas').on('click', function(evt){
  ...
  var bgcolor = can.backgroundColor;
  var background_color = ((can.backgroundColor) ||"white");
  console.log(background_color);
  console.log(can.backgroundColor);
  console.log(JSON.stringify(can));
  $.ajax({
      type: 'post',
      url: url, 
      data : { '_token': token, 'canvas' : JSON.stringify(can), 'title':title, 'aspect_ratio':aspect_ratio, 'orientation':orientation,'content_type':content_type, 'category':category, 'background_color':background_color},
      success:function(data){
          window.location="{{url('welcome')}}";
      
      },
      error: function(){
        alert("Layout name is empty");
