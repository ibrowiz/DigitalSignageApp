...
$('#save-canvas').on('click', function(evt){
  ...
  var url = "{{ url('mediapartner/'.$tenant->domain.'/create-canvas')}}";
  ...
  $.ajax({
    type: 'post',
    url: url, 
    data : { '_token': token, 'canvas' : JSON.stringify(can), 'image':dataURL ,'file_name':file_name, 'orientation':orientation},
    success:function(data){
        window.location="{{url('mediapartner/'.$tenant->domain.'/layouttemplates')}}";
