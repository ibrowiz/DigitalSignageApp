...
@section('content')
 ...
 <div class="spacer">
        <div>
            <a href="{{URL::to('mediapartner/'.$tenant->domain.'/layouttemplates')}}"> 
                      ...
                      @if(count($layouts) > 0 )
                        @foreach($layouts->all() as $layout)
                          <tr>
                            ...
                            <td>{{$layout->title}}</td>
                            <td>
                              <a href='{{url("/mediapartner/{$tenant->domain}/layout/view/{$layout->id}")}}' class="label label-primary"> view</a> <a href= '{{url("/mediapartner/{$tenant->domain}/layout/destroy/{$layout->id}")}}' class="label label-danger">delete</a>|
                            </td>
                          </tr>
                        @endforeach
                      @endif
