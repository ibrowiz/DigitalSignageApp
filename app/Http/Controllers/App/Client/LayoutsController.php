...
class LayoutsController extends Controller
{
    ...
    {
        $this->middleware(['auth', 'cors'])->except('preview');
    }
    ...
    public function create($domain,Request $request){
        ...
        $layout = layout::create(['screen' => $request->canvas, 'assignable_id'=>Auth::user()->client->id, 'assignable_type' => 'App\Models\Client', 'orientation'=>$request->orientation,'background_color'=>$request->background_color, 'title'=>$request->title, 'aspect_ratio'=>$request->aspect_ratio]);
        //\Log::info($user);
    ...
    public function show($domain){
        header("Access-Control-Allow-Origin: *");
    	//$layouts = Layout::where('assignable_id', Auth::user()->client->id)->get();
        $layouts = Auth::user()->client->layouts;
        // $layouts = Layout::order_by('title','desc')->paginate(5);
        return view ('app.client.layouts.welcome', ['layouts'=>$layouts]);
    }

    public function read($domain,$id, Request $request){
        
    	$layout = Layout::find($id);
        $resources = Resource::where('assignable_id', Auth::user()->client->id)->get();
        $contents = Content::all();
        //dd($contents);
    	return view('app.client.layouts.view', ['layout'=>$layout, 'resources'=>$resources, 'contents'=>$contents]);
    }

    public function edit($domain,$id, Request $request){

        $data = array('screen' => $request->canvas, 'assignable_id'=>Auth::user()->client->id, 'orientation'=>$request->orientation,'background_color'=>$request->background_color, 'title'=>$request->title, 'aspect_ratio'=>$request->aspect_ratio);
    ...
    public function destroy($domain,$id){
        ...
        return redirect('client/'.$domain.'/layout/welcome')->with('info', 'Layout deleted successfully');
    }
