...
class LayoutsController extends Controller
{
    ...
    {
        $this->middleware(['auth:operator', 'cors'])->except('preview');
    }
    ...
    public function create($domain,Request $request){
        ...
        $layout = layout::create(['screen' => $request->canvas, 'assignable_id'=>Auth::user()->tenant->id, 'assignable_type' => 'App\Models\Tenant', 'orientation'=>$request->orientation,'background_color'=>$request->background_color, 'title'=>$request->title, 'aspect_ratio'=>$request->aspect_ratio]);
        //\Log::info($user);
    ...
    public function show(){
        header("Access-Control-Allow-Origin: *");
    	//$layouts = Layout::where('assignable_id', Auth::user()->tenant->id)->get();
        $layouts = Auth::user()->tenant->layouts;
        // $layouts = Layout::order_by('title','desc')->paginate(5);
        return view ('app.mediapartner.layouts.welcome', ['layouts'=>$layouts]);
    }
    ...
    public function read($domain,$id, Request $request){
        ...
        //dd($contents);
    	return view('app.mediapartner.layouts.view', ['layout'=>$layout, 'resources'=>$resources, 'contents'=>$contents]);
    }

    public function edit($domain,$id, Request $request){

        $data = array('screen' => $request->canvas, 'assignable_id'=>Auth::user()->tenant->id, 'assignable_type' => 'App\Models\Tenant', 'orientation'=>$request->orientation,'background_color'=>$request->background_color, 'title'=>$request->title, 'aspect_ratio'=>$request->aspect_ratio);
    ...
    public function destroy($domain,$id){
        ...
        return redirect('mediapartner/'.$domain.'/layout/welcome')->with('info', 'Layout deleted successfully');
    }
